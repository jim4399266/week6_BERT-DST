...
from dataset_cn import Dataset_cn, Ontology_cn
...
def create_parser():
    ...
    parser.add_argument('--language', type=str, default='en')    # 数据集的语言，默认英文数据集

    return parser

def load_dataset_cn(base_path):
    dataset = {}
    dataset['train'] = Dataset_cn.from_dict(read_json(base_path / 'readable_train_data.json'))
    dataset['dev'] = Dataset_cn.from_dict(read_json(base_path / 'readable_val_data.json'))
    dataset['test'] = Dataset_cn.from_dict(read_json(base_path / 'readable_test_data.json'))
    ontology = Ontology_cn.from_dict(read_json(base_path / 'ontology.json'))
